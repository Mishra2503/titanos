      ...
      {tab === "posts" && (
        <div className="mt-4 space-y-4">
          ...
          {detail.posts.length === 0 ? (
            <p className="font-mono text-sm text-ink-faint">No saved posts yet. Add their best content to analyze hashtags and formats.</p>
          ) : (
            <div className="space-y-2">
              {detail.posts.map((p) => (
                <div key={p.id} className="rounded-lg border border-charcoal-700 bg-charcoal p-3">
                  <div className="flex items-center justify-between font-mono text-[10px] text-ink-faint">
                    <span>
                      {p.post_type || "POST"}
                      {p.posted_on ? ` · ${p.posted_on}` : ""}
                      {p.engagement != null ? ` · ${fmt(p.engagement)} interactions` : ""}
                    </span>
                    <div className="flex gap-2">
                      {p.permalink && (
                  ...
                  {p.caption && <p className="mt-1 whitespace-pre-wrap text-xs text-ink-muted">{p.caption}</p>}
                  {p.hashtags.length > 0 && (
                    <p className="mt-1 font-mono text-[10px] text-lime-dim">{p.hashtags.join(" ")}</p>
                  )}
                  {p.what_works && (
                    <p className="mt-1 text-[11px] text-ink-faint">
                      <span className="text-ink-muted">Why it works:</span> {p.what_works}
                    </p>
                  )}
                </div>
              ))}
...
function PostForm({ onSubmit }: { onSubmit: (b: PostInput) => Promise<void> }) {
  const [f, setF] = useState({ type: "REEL", permalink: "", caption: "", likes: "", comments: "", posted_on: "", what_works: "" });
  const [saving, setSaving] = useState(false);

  return (
    <div className="rounded-lg border border-charcoal-700 bg-charcoal p-3">
      <p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-ink-faint">Save a reference post</p>
      ...
      <input placeholder="Permalink (optional)" className={`${inputCls} mt-2`} value={f.permalink} onChange={(e) => setF({ ...f, permalink: e.target.value })} />
      <textarea placeholder="Caption (hashtags are auto-extracted)" rows={2} className={`${inputCls} mt-2`} value={f.caption} onChange={(e) => setF({ ...f, caption: e.target.value })} />
      <input placeholder="Why it works (the takeaway to replicate)" className={`${inputCls} mt-2`} value={f.what_works} onChange={(e) => setF({ ...f, what_works: e.target.value })} />
      <button
        onClick={async () => {
          setSaving(true);
          await onSubmit({
            ...
            what_works: f.what_works || null,
          });
          setF({ type: f.type, permalink: "", caption: "", likes: "", comments: "", posted_on: "", what_works: "" });
          setSaving(false);
        }}
        disabled={saving}
        className="press mt-2 rounded-lg border border-charcoal-600 px-3 py-1.5 text-xs text-ink-muted hover:text-ink disabled:opacity-50"
