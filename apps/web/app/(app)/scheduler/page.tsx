...
export default function SchedulerPage() {
  ...
  function handleFile(f: File | null) {
    ...
    v.onloadedmetadata = () => {
      ...
      setLocalSpecs({ ok: true, duration, width, height });
    };
  }
      ...
      {tab === "compose" && (
        <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
          ...
          <div className="space-y-4">
            ...
            {localSpecs && (
              <div className="animate-reveal rounded-lg border border-charcoal-600 bg-charcoal-700/40 px-3 py-2 text-xs text-ink-muted">
                {localSpecs.duration?.toFixed(1)}s
                {localSpecs.width && localSpecs.height ? ` · ${localSpecs.width}×${localSpecs.height}` : ""}
                {" · "}{file?.name}
              </div>
            )}
            ...
            <div>
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <p className="font-mono text-[10px] uppercase tracking-wider text-ink-faint">
                  Target accounts ({selected.length}/{accounts.length})
                </p>
                <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        accounts.forEach((a, i) => {
                          ...
                          next[a.id] = rows[a.id] ?? { ig_account_id: a.id, caption: "", hashtags: [], scheduled_at: toLocalInput(t) };
                        });
                  ...
                  {selected.length > 0 && (
                    <button
                      onClick={() => { setSelected([]); setRows({}); }}
                      className="press font-mono text-[10px] uppercase tracking-wider text-ink-faint hover:text-ink"
                    >
                      Clear
                    </button>
                  )}
