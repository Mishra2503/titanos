...
async function graphGet(path: string, token: string) {
  const r = await fetch(`${GRAPH}${path}&access_token=${token}`, { signal: AbortSignal.timeout(15000) });
...
async function buildAccountInsights(account: { id: string; igUserId: string; username: string; followersCount: number | null; accessTokenEnc: string }) {
  ...
  let token: string;
  try { token = decryptSecret(account.accessTokenEnc); } catch { return null; }

  const [acctMetrics, mediaList] = await Promise.allSettled([
    graphGet(`/${account.igUserId}/insights?metric=profile_views&period=day&since=${Math.floor((Date.now() - 28*86400000)/1000)}&until=${Math.floor(Date.now()/1000)}`, token),
    graphGet(`/${account.igUserId}/media?fields=id,caption,permalink,thumbnail_url,media_url,timestamp,media_product_type&limit=${MAX_MEDIA}`, token),
  ]);

  const profileViews = acctMetrics.status === "fulfilled" ? (acctMetrics.value?.data?.[0]?.values?.reduce((s: number, v: { value: number }) => s + (v.value || 0), 0) ?? null) : null;
  ...
  for (const r of postInsights) {
    ...
    likes += vals.likes ?? 0; comments += vals.comments ?? 0;
    const inter = vals.total_interactions ?? ((vals.likes ?? 0) + (vals.comments ?? 0) + (vals.shares ?? 0) + (vals.saved ?? 0));
    ...
    recentPosts.push({
      ...
      likes: vals.likes ?? null, comments: vals.comments ?? null,
      shares: vals.shares ?? null, saved: vals.saved ?? null,
      avg_watch_time_sec: vals.ig_reels_avg_watch_time ? vals.ig_reels_avg_watch_time / 1000 : null,
      total_watch_time_sec: vals.ig_reels_video_view_total_time ? vals.ig_reels_video_view_total_time / 1000 : null,
  ...
  const result = {
    ...
    followers: account.followersCount, reach: reachSum,
    profile_views: profileViews, saves, shares, likes, comments,
    engagement_rate: reachSum > 0 ? Math.round((interSum / reachSum) * 1000) / 10 : null,
    posts_analyzed: media.length, recent_posts: recentPosts,
  };
  setCache(account.id, result);
  return result;
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    type AccountResult = { account_id: string; username: string; followers: number | null; reach: number; profile_views: number | null; saves: number; shares: number; likes: number; comments: number; engagement_rate: number | null; posts_analyzed: number; recent_posts: unknown[] };
